...
Scenario('The Hoover is sent to clean one patch of dirt on a 5x5 grid', ({ I }) => {
    ...
    // Then I should see that one patch was cleaned
...
Scenario('The Hoover is sent to clean two patches of dirt on a 5x5 grid', ({ I }) => {
    ...
    // Then I should see that two patches were cleaned
    I.seeResponseCodeIsSuccessful();
    I.seeResponseContainsJson(expectedJson)
});
...
Scenario('The Hoover is sent to clean three patches of dirt on a 5x5 grid', ({ I }) => {
    ...
    // Then I should see that three patches were cleaned
    I.seeResponseCodeIsSuccessful();
    I.seeResponseContainsJson(expectedJson)
});
...
Scenario('The Hoover runs into walls while cleaning dirt patches on a 5x5 grid', ({ I }) => {
    ...
    // Then I should see that the Hoover correctly navigated the room and cleaned three patches
    I.seeResponseCodeIsSuccessful();
    I.seeResponseContainsJson(expectedJson)
});

Scenario('The Hoover is navigated across the entirety of a 5x5 grid', ({ I }) => {
...
Scenario('The starting coordinates of the Hoover are outside of the defined 5x5 grid size', ({ I }) => {
...
Scenario('The network request payload has only one coordinate defined in a nested "patches" array', ({ I }) => {

    // When the following network request is sent
    I.sendPostRequest(urlPath, { "roomSize": [5, 5], "coords": [1, 0], "patches": [[1], [2, 2], [2, 3]], "instructions": "SSENNNW" })
...
Scenario('The network request payload has a float value instead of an integer in a nested "patches" array', ({ I }) => {

    // When the following network request is sent
    I.sendPostRequest(urlPath, { "roomSize": [5, 5], "coords": [1, 0], "patches": [[1, 0], [2, 2.2], [2, 3]], "instructions": "SSENNNW" })

    // Then I expect to see a client error based on the float value being present in the nested "patches" array
    I.seeResponseCodeIsClientError();
});
...
Scenario('The "roomSize" value is set to 0, 0', ({ I }) => {
    ...
    // Then I expect to see a client error based on the "roomSize" value set to 0, 0 which is invalid
    I.seeResponseCodeIsClientError();
});
...
Scenario('The roomSize is set to an extremely large number to see the maximum supported value', ({ I }) => {
    ...
    // Then I expect to see a client error based on the "roomSize" value set to something that the server does not support
    I.seeResponseCodeIsClientError();
});
...
Scenario('The network request is sent to the wrong API path', ({ I }) => {

    // When the following network request is sent
    I.sendPostRequest('v1/cleaning-session', { "roomsize": [5, 5], "coords": [1, 0], "patches": [[1, 0], [2, 2], [2, 3]], "instructions": "SSENNNW" })

    // Then I expect to see a 404 error based on the request being sent to an API path that does not exist
    I.seeResponseCodeIs(404);
});

// Scenario('The Hoover is navigated across the entirety of a 5x5 grid with each sector being defined as a dirt patch', ({ I }) => {
...
//     // Then I should see that the Hoover correctly navigated the room and cleaned twenty four patches
//     I.seeResponseCodeIsSuccessful();
//     I.seeResponseContainsJson(expectedJson)
// });